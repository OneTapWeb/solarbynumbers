...
Our [export still isn't certified](/tariff/), so we currently earn nothing for the surplus we
push to the grid. Axle is different: it settles **directly through the smart meter** over the
...
re-registering the swapped hardware to our account. A support ticket and a little patience later,
...
This was the part we expected to be fiddly and turned out not to be. **Predbat has Axle support
...
first-party Axle → Home Assistant integration that surfaces events as a sensor, which is handy
for the dashboard.
